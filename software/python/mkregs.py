...
#TODO: TOP and CORE_PREFIX are the same; remove core prefix
def print_usage():
...
def write_hwheader(table, regfile_name):
    ...
    fout.write("`define " + regfile_name + "_ADDR_W " + str(get_core_addr_w(table)) + "\n\n")
...
def swreg_parse (code, hwsw, regfile_name, core_prefix):
    swreg_addr = 0
...
def main () :
    ...
    if len(sys.argv) < 3:
        ...
        regfile_name = sys.argv[1]
        hwsw = sys.argv[2]

    #TODO: code below is redundant: only TOP needed
    core_prefix = ""
    if hwsw == "SW":
        try:
    ...
    #parse input file
    fin = open (regfile_name, 'r')
    defsfile = fin.readlines()
    fin.close()

    regfile_name = regfile_name.split('/')[-1].split('.')[0]

    swreg_parse (defsfile, hwsw, regfile_name, core_prefix)
