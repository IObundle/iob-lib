...
def write_swheader(table, regvfile_name, core_prefix, defines):
    ...
    fout.write("//This file was generated by script mkregs.py\n\n")
    fout.write("#ifndef H_IOB_UART_SWREG_H\n")
    fout.write("#define H_IOB_UART_SWREG_H\n\n")
...
def write_sw_pc_emul(table, regvfile_name, core_prefix, defines):
    ...
    swheader_name = regvfile_name+'.h'
    fout.write(f"#include \"{swheader_name}\"\n")
    pc_emul_header = regvfile_name+'_pc_emul.h'
    fout.write(f"#include \"{pc_emul_header}\"\n\n")
...
def write_sw_pc_emul_header(table, regvfile_name, core_prefix, defines):
    ...
    fout.write("//This file was generated by script mkregs.py\n\n")
    fout.write("#ifndef H_IOB_UART_SWREG_PC_EMUL_H\n")
    fout.write("#define H_IOB_UART_SWREG_PC_EMUL_H\n\n")
    fout.write("#include <stdint.h>\n\n")

    fout.write("// SWREG Types\n")
    for row in table:
        name = row[0]
        width = row[3]
        sw_type = swreg_type(width, defines)
    ...
    fout.write("\n// PC Emulation of each SWREG functionality\n")
    fout.write("// functions to be implemented by sw driver developer\n")
    for row in table:
        read_write = row[1]
        if read_write == "W":
            name = row[0]
            fout.write(f"void pc_emul_set_{name.lower()}(int64_t value);\n")
    for row in table:
        read_write = row[1]
        if read_write == "R":
            name = row[0]
