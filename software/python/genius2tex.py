...
def process_reports():
    ...
    if(not os.path.exists(gates_rpt)): return -1
    area       = float(run_command("grep -m1 'total' " + gates_rpt + " | awk '{print $3}'"))
    nands      = int(run_command("grep -m1 'ND2CLD' " + gates_rpt + " | awk '{print $2}'"))
    nands_area = float(run_command("grep -m1 'ND2CLD' " + gates_rpt + " | awk '{print $3}'"))
    ffs        = int(run_command("grep -m1 'sequential' " + gates_rpt + " | awk '{print $2}'"))
    
    # Process timing report
    if(not os.path.exists(timing_rpt)): return -1
    per   = int(run_command("grep -m1 'capture' " + timing_rpt + " | awk '{print $4}'"))
    slack = int(run_command("grep -m1 'slack' " + timing_rpt + " | awk '{print $4}' | grep -o '[^a-z]*'"))
    
    return 0

def generate_output_file():
    fw = open(output_file, "w")
    fw.write(str(area) + " & " + str(gates) + " & " + str(ffs) + " & " + '{0:.5g}'.format(freq) + "\\\\ \\hline")
    fw.close()
    return
