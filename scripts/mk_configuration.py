...
def config_for_board(top, flows_list, build_dir):
    available_configs = {
        ...
        'SIMULATION':{'BAUD':'3000000', 'FREQ':'100000000', 'MEM_NO_READ_ON_WRITE':'0', 'DDR_DATA_W':'32', 'DDR_ADDR_W':'24'}
        }
    ...
    for name, config in available_configs.items():
        ...
        file.write(f"$(shell echo '`define {top.upper()}_BAUD {config['BAUD']}' >> ../src/build_configuration.vh)\n")
        file.write(f"$(shell echo '`define {top.upper()}_FREQ {config['FREQ']}' >> ../src/build_configuration.vh)\n")
        file.write(f"$(shell echo '`define {top.upper()}_MEM_NO_READ_ON_WRITE {config['MEM_NO_READ_ON_WRITE']}' >> ../src/build_configuration.vh)\n")
        file.write(f"$(shell echo '`define {top.upper()}_DDR_DATA_W {config['DDR_DATA_W']}' >> ../src/build_configuration.vh)\n")
        file.write(f"$(shell echo '`define {top.upper()}_DDR_ADDR_W {config['DDR_ADDR_W']}' >> ../src/build_configuration.vh)\n")
        ...
        if 'emb' in flows_list:
            ...
            file.write(f"$(shell echo '#define BAUD {config['BAUD']}' >> ../../software/esrc/build_configuration.h)\n")
            file.write(f"$(shell echo '#define FREQ {config['FREQ']}' >> ../../software/esrc/build_configuration.h)\n")
            file.write(f"$(shell echo '#define MEM_NO_READ_ON_WRITE {config['MEM_NO_READ_ON_WRITE']}' >> ../../software/esrc/build_configuration.h)\n")
            file.write(f"$(shell echo '#define DDR_DATA_W {config['DDR_DATA_W']}' >> ../../software/esrc/build_configuration.h)\n")
            file.write(f"$(shell echo '#define DDR_ADDR_W {config['DDR_ADDR_W']}' >> ../../software/esrc/build_configuration.h)\n")
