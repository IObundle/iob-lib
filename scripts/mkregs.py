...
def write_hwcode(table, o_dir, top):

    #
    # SWREG INSTANCE
    #

    fswreg_inst = open(f"{o_dir}/{top}_swreg_inst.vh", "w")
    ...
    fswreg_gen = open(f"{o_dir}/{top}_swreg_gen.v", "w")
...
def write_hwheader(table, o_dir, top):
    fswreg_def = open(f"{o_dir}/{top}_swreg_def.vh", "w")
...
def write_swheader(table, o_dir, top):
    fswhdr = open(f"{o_dir}/{top}_swreg.h", "w")
...
def write_sw_emb(table, o_dir, top):
    fsw = open(f"{o_dir}/{top}_swreg_emb.c", "w")
    core_prefix = f"{top}_".upper()
...
def swreg_proc(toml_dict, hwsw, top, o_dir):
    table = swreg_list(toml_dict)
    if hwsw == "HW":
        write_hwheader(table, o_dir, top)
        write_hwcode(table, o_dir, top)
    elif hwsw == "SW":
        write_swheader(table, o_dir, top)
        write_sw_emb(table, o_dir, top)
...
def main():
    ...
    fin.close()
    swreg_proc(toml_dict, args.hwsw, args.TOP, args.o_dir)
