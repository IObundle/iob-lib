...
def gen_wr_reg(row, f):
    ...
    f.write(f"\n`IOB_WIRE({reg}_wen, 1)\n")
    f.write(f"assign {reg}_wen = valid_i & (|wstrb_i[{byte_offset}+:{row['nbytes']}]) & (`IOB_WORD_ADDR(addr_i) >= `IOB_WORD_ADDR({reg_addr})) & (`IOB_WORD_ADDR(addr_i) < `IOB_WORD_CADDR({reg_addr + 2**reg_addr_w -1}));\n")
...
def gen_rd_reg(row, f):
    ...
    reg_addr_w = row['addr_w']
    f.write(f"\n`IOB_WIRE({reg}_ren, 1)\n")
    f.write(f"assign {reg}_ren = valid_i & ( `IOB_WORD_ADDR(addr_i) >= `IOB_WORD_ADDR({reg_addr}) ) & ( `IOB_WORD_ADDR(addr_i) < `IOB_WORD_CADDR({reg_addr + 2**reg_addr_w -1}) ) & ~(|wstrb_i);\n")
    if row['autologic']:
        f.write(f"`IOB_WIRE({reg}_ready_i, 1)\n")
        f.write(f"assign {reg}_ready_i = !wstrb_i;\n")
...
def write_hwcode(table, top):
    ...
    for row in table:
        ...
        if row['rw_type'] == 'R':
            ...
            fswreg_gen.write(f"\tif( (`IOB_WORD_ADDR(addr_r) >= `IOB_WORD_ADDR({row['addr']})) & (`IOB_WORD_ADDR(addr_r) < `IOB_WORD_CADDR({row['addr'] + 2**row['addr_w'] -1}) )"+" begin\n")
            # get rdata
            if row['autologic']:
            ...
            # get rready
            fswreg_gen.write(f"\tif( (`IOB_WORD_ADDR(addr_i) >= `IOB_WORD_ADDR({row['addr']})) & (`IOB_WORD_ADDR(addr_i) < `IOB_WORD_CADDR({row['addr'] + 2**row['addr_w'] -1}))\n")
...
def swreg_parse(code, hwsw, top, vh_files):
    ...
    if hwsw == "HW":
        ...
        write_swheader(table, top)
        write_sw_emb(table, top)
