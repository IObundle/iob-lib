...
import if_gen
from submodule_utils import get_submodule_directories, get_module_io, import_setup, get_pio_signals, if_gen_interface, find_suffix_from_list
...
def get_interface_mapping(full_if_name):
    ...
    assert False, f"{iob_colors.FAIL} Unknown mapping for ports of '{full_if_name}' interface.{iob_colors.ENDC}"
    ...
    for table in ios:
        ...
        if if_name:
            ...
            for port in table['ports']:
                f_io.write(f"{get_port_type(port['type'])} [{port['n_bits']}-1:0] {table['name']+'_' if ios_table_prefix else ''}{port['name']}, //{port['descr']}\n")
