...
def set_default_submodule_dirs(meta_data):
    ...
    if 'LIB' not in meta_data['submodules']['dirs']:
        meta_data['submodules']['dirs']['LIB'] = "submodules/LIB"
...
def iob_soc_peripheral_setup(python_module, append_peripheral_ios=True):
    ...
    if peripherals_list:
        ...
        _, params_list, top_list = get_peripherals_ports_params_top(peripherals_list, python_module.meta['submodules']['dirs'])
    ...
    if peripherals_list and append_peripheral_ios: python_module.ios.extend(get_peripheral_ios(peripherals_list, python_module.meta['submodules']))
...
def get_peripherals_ports_params_top(peripherals_list, submodule_dirs):
    ...
    for instance in peripherals_list:
        if instance['type'] not in port_list:
            ...
            set_default_submodule_dirs(module.meta)
