...
class iob_lib(iob_module):
    ...
    def _run_setup(cls):
        ...
        shutil.copyfile(
            os.path.join(cls.setup_dir, "iob_lib.vh"),
            os.path.join(cls.build_dir, out_dir, "iob_lib.vh"),
        )
        ...
        if cls._setup_purpose[-1]=='hardware' and len(cls._setup_purpose)>1:
            ...
            for purpose in [x for x in cls.cls._setup_purpose[:-1] if x!="software"]:
                # Delete sources for this purpose
                os.remove(os.path.join(cls.build_dir, cls.cls.PURPOSE_DIRS[purpose], "iob_lib.vh"))
