...
class iob_fifo_sync(iob_module):
    ...
    def _run_setup(cls):
        ...
        shutil.copyfile(
            os.path.join(cls.setup_dir, "iob_fifo_sync.v"),
            os.path.join(cls.build_dir, out_dir, "iob_fifo_sync.v"),
        )
        ...
        if cls._setup_purpose[-1]=='hardware' and len(cls._setup_purpose)>1:
            ...
            for purpose in [x for x in cls._setup_purpose[:-1] if x!="software"]:
                ...
                os.remove(os.path.join(cls.build_dir, cls.PURPOSE_DIRS[purpose], "iob_fifo_sync.v"))
