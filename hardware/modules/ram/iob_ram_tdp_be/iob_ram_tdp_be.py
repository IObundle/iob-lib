...
class iob_ram_tdp_be(iob_module):
    ...
    def _run_setup(cls):
        ...
        if cls.is_top_module:
            ...
            shutil.copyfile(
                os.path.join(cls.setup_dir, "iob_ram_tdp_be_tb.v"),
                os.path.join(
                    cls.build_dir, "hardware/simulation/src", "iob_ram_tdp_be_tb.v"
                ),
            )
    ...
    def _copy_srcs(cls):
        ...
        if cls._setup_purpose[-1] == "hardware" and len(cls._setup_purpose) > 1:
            ...
            for purpose in [x for x in cls._setup_purpose[:-1] if x != "software"]:
                ...
                os.remove(
                    os.path.join(
                        cls.build_dir, cls.PURPOSE_DIRS[purpose], "iob_ram_tdp_be.v"
                    )
                )
